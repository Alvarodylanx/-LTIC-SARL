      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-wrap items-center justify-center gap-4 sm:gap-6">
        {active.map(({ key, Icon, label, color }, i) => (
          <motion.a
            ...
            whileTap={{ scale: 0.92 }}
            className={`text-sidebar-foreground/40 transition-colors duration-200 ${color}`}
          >
        ...
        {hasWhatsApp && (
          <motion.a
            ...
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: active.length * 0.05, duration: 0.3 }}
            whileHover={{ scale: 1.2, y: -2 }}
            whileTap={{ scale: 0.92 }}
            className="text-sidebar-foreground/40 hover:text-green-400 transition-colors duration-200"
          >
            <WhatsAppSVG />
          </motion.a>
        )}
