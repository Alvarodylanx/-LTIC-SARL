...
function Get-NextVersion {
    ...
    $sorted = $tags | Where-Object { $_ -match '^v\d+\.\d+\.\d+$' } |
              Sort-Object { [version]($_ -replace '^v','') }
    if (-not $sorted) { return "v1.1.0" }
...
switch ($Command) {
    ...
    "save" {
        ...
        Write-Host "  To restore later: .\version.ps1 restore $version"
        Write-Host ""
    }
    ...
    "show" {
        if (-not $Arg) {
            Write-Host "ERROR: Provide a version. Example: .\version.ps1 show v1.0.0" -ForegroundColor Red
            exit 1
        }
        Write-Host ""
    ...
    "diff" {
        if (-not $Arg) {
            Write-Host "ERROR: Provide a version. Example: .\version.ps1 diff v1.0.0" -ForegroundColor Red
            exit 1
        }
        Write-Host ""
        ...
        Write-Host ""
        $answer = Read-Host "Show full line-by-line diff? (y/n)"
        if ($answer -eq "y") { git diff $Arg HEAD }
    }

    "restore" {
        if (-not $Arg) {
            Write-Host "ERROR: Provide a version. Example: .\version.ps1 restore v1.0.0" -ForegroundColor Red
            exit 1
        }

        $tagExists = git tag --list $Arg 2>$null
        if (-not $tagExists) {
        ...
        # Stash unsaved changes so nothing is lost
        ...
        # Restore files to the chosen version and create a new commit (history stays intact)
        git checkout $Arg -- .
        git add .
        git commit -m "restore: reverted to $Arg"
        ...
        if ($stashed) {
            ...
            Write-Host "  To recover them run: git stash pop" -ForegroundColor DarkGray
        }
        Write-Host ""
        Write-Host "  IMPORTANT: Rebuild the db package after restoring:" -ForegroundColor Yellow
    ...
    "status" {
        ...
        Write-Host "  Latest saved version: $current"
        Write-Host ""
    ...
    default {
        ...
        Write-Host "    .\version.ps1 diff v1.0.0         - compare current code to that version"
        Write-Host "    .\version.ps1 restore v1.0.0      - revert code to that version"
        Write-Host ""
        Write-Host "  Examples:"
        Write-Host "    .\version.ps1 save 'added contact form validation'"
        Write-Host "    .\version.ps1 restore v1.0.0"
        Write-Host ""
    }
}
